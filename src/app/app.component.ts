...
export class AppComponent {
  ...
  createDatabase(){
    this.sqlite.create({name:'hlb_db.db',location:'default'}).then((db:SQLiteObject)=>{
      ...
      this.userLocalService.setDatabase(db);
    }).then(()=>{
      //Create first table
      ...
      return this.userLocalService.createTable();
    }).then(()=>{
      this.splashScreen.hide();
    }).catch((error)=>{
  ...
  setDefaultUser(){
    this.almacenamientoNativoService.almacenarUsuarioPorDefault({fullName:"Area TI",username:"areati",password:'areati',actions:[USER_ACTIONS.ACCESO_A_CONFIGURACIONES,
      USER_ACTIONS.EDITAR_REGISTROS_TRAMPAS,USER_ACTIONS.AGREGAR_REGISTROS_TRAMPAS,
      USER_ACTIONS.EDITAR_REGISTROS_TRASPATIOS_FINCAS,USER_ACTIONS.AGREGAR_REGISTROS_TRASPATIOS_FINCAS,
      USER_ACTIONS.EDITAR_REGISTROS_INSP_TRASPATIOS_FINCAS,USER_ACTIONS.AGREGAR_REGISTROS_INSP_TRASPATIOS_FINCAS,
      USER_ACTIONS.EDITAR_REGISTROS_INSP_TRAMPAS,USER_ACTIONS.AGREGAR_REGISTROS_INSP_TRAMPAS,USER_ACTIONS.EDITAR_MARCA_A_INSPECCION
  ],token:''}).then((usuario)=>{
  ...
  logueoPaPruebas(){
    let user = {
      ...
      password:"Az123456",
      actions:[USER_ACTIONS.ACCESO_A_CONFIGURACIONES,
        USER_ACTIONS.EDITAR_REGISTROS_TRAMPAS,USER_ACTIONS.AGREGAR_REGISTROS_TRAMPAS,
        USER_ACTIONS.EDITAR_REGISTROS_TRASPATIOS_FINCAS,USER_ACTIONS.AGREGAR_REGISTROS_TRASPATIOS_FINCAS,
        USER_ACTIONS.EDITAR_REGISTROS_INSP_TRASPATIOS_FINCAS,USER_ACTIONS.AGREGAR_REGISTROS_INSP_TRASPATIOS_FINCAS,
        USER_ACTIONS.EDITAR_REGISTROS_INSP_TRAMPAS,USER_ACTIONS.AGREGAR_REGISTROS_INSP_TRAMPAS,USER_ACTIONS.EDITAR_MARCA_A_INSPECCION
